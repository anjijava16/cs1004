...
public class Main {
	...
	public static void main(String[] args) {
		System.out
				.println("What pitch is this? Type what pitch you think it is.");
		while (true) {
			int pitchNumber = randomGen.nextInt(12);
			// The commented code below will tell you what note is played, so
			// you can test what happens when the user answers correctly.
			// System.out.println(PlaySound.files[pitchNumber]);
			while (true) {
				PlaySound.playPitch(pitchNumber);
				System.out
						.println("Type 'Play pitch again' to hear the pitch another time.");
				System.out
						.println("Type 'I give up' if you want the answer, or 'Exit' if you want to stop.");
				...
				if (Guess.isUserRight(userGuess, pitchNumber)) {
					...
					PlaySound.playSequence();
					break;
				} else if (userGuess.equals("Play pitch again")) {} else if (userGuess
						.equals("Exit")) {
